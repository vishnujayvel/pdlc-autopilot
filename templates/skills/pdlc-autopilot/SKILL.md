...
Phase 0a: Auto-generate missing artifacts (MUST invoke Kiro skills — PROC-1)
    ...
    → DO NOT use general-purpose subagents to write these artifacts (PROC-1 violation)
    → Record provenance in progress.md (see PROC-1 constraint)
    → Runtime error: output cc-sdd install instructions, STOP

Phase 0b: Dual-Perspective + Product Skeptic + Kiro Validation (PROC-1)
    → STEP 1 — Kiro Validation (MUST invoke via Skill tool — PROC-1):
...
| Gap Analysis | kiro:validate-gap | **Skill tool** (PROC-1) | Warnings only |
| Design | kiro:validate-design | **Skill tool** (PROC-1) | Yes if NO-GO |
...
| Per-Batch | Critic ADVOCATE + SKEPTIC | Task tool (2 parallel subagents) — **PROC-2** | Yes if BOTH FAIL |
| Final | Final Validator | Task tool (2 parallel + PROC-1 check) | Reports gaps |
| PR Review | Review Comment Actors | Task tool (batched by file) — **PROC-2** | Max 2 cycles |
| P2 Docs | Docs Critic | Task tool (1 subagent) | Opt-in only |

**IMPORTANT:** Rows marked **Skill tool (PROC-1)** MUST be invoked using the Skill tool, NOT the Task tool. See PROC-1 constraint above.

## Director Protocol

### Step 0: Auto-Generate & Validate Spec (PROC-1 Enforcement Point)
...
3. Phase 0a — Check for required files and AUTO-GENERATE if missing:
   ┌─────────────────────────────────────────────────────────────────┐
   │ PROC-1 MANDATORY: Use the Skill tool for ALL artifact generation│
   │                                                                  │
...
5. Phase 0b — Kiro Validation (PROC-1 MANDATORY):
   ┌─────────────────────────────────────────────────────────────────┐
   │ PROC-1 MANDATORY: Use the Skill tool for Kiro validation       │
   │                                                                  │
...
For each batch:
  ...
  ┌─────────────────────────────────────────────────────────────────┐
  │ PROC-2 MANDATORY: Critic dispatch after EVERY Actor batch       │
  │                                                                  │
...
### PROC-1: Kiro Skill Invocation Is MANDATORY (BLOCKING)
...
is a PROC-1 violation. Subagents may READ and VALIDATE artifacts, but Kiro skills
MUST generate them.
```
...
instead of Kiro validation skills is a PROC-1 violation.
...
### PROC-2: Per-Batch Critic Review Is MANDATORY (BLOCKING)
...
- Skip Critic review
- Mark a batch "DONE" without both ADVOCATE and SKEPTIC critic results (PROC-2 violation)
- Skip critics for "simple" batches — all batches get critics regardless of artifact type (PROC-2 violation)
- Run parallel batches that touch same file
- Let two teammates modify the same file (T-Mode)
- Use general-purpose subagents to WRITE spec artifacts (PROC-1 violation)
- Skip Kiro validation skills and substitute custom subagent prompts (PROC-1 violation)
...
**Kiro Skills (PROC-1 — MUST invoke via Skill tool, NOT Task tool):**
